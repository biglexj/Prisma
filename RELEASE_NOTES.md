...
### Resumen
Presentamos **Prisma v1.1.0**, una actualización mayor que introduce el **Convertidor Multimedia por Lotes** impulsado por el motor nativo de FFmpeg, incorpora tratamiento especializado de **Audio Lossless de Máxima Fidelidad (FLAC y WAV)**, despliega los nuevos selectores personalizados **CustomSelect (Material 3 Expressive)** en toda la suite, unifica el acceso al **Ecualizador DSP y Selector Físico de Salidas de Audio** en el reproductor de vídeo con sincronización en tiempo real, y perfecciona la ergonomía visual del visor con posicionamiento dinámico de notificaciones y optimizaciones en Quick Look.

### Detalles
- **Convertidor Multimedia por Lotes (Motor Nativo FFmpeg)**: Suite integral de conversión y procesamiento de medios en segundo plano para archivos individuales o carpetas enteras:
  - **Vídeo a Audio**: Extracción directa de bandas sonoras a MP3, FLAC, WAV, AAC, OGG y M4A con selección granular de canales de audio (estéreo o mono).
  - **Conversor de Vídeo**: Transcodificación multiformato a contenedores MP4, MKV y WebM, con soporte de aceleración y códecs modernos H.264 (máxima compatibilidad), HEVC / H.265 (alta compresión), AV1 (nueva generación ultra eficiente) y copia de stream directo sin recodificar (*stream copy*), además de reescalado opcional a 1080p, 720p o 480p preservando la relación de aspecto.
  - **Transcodificador de Audio**: Conversión entre formatos acústicos con control exhaustivo de calidad y reglas apiladas de renombrado por lote (prefijos, sufijos y sustitución inteligente de texto).
- **Tratamiento Especializado para Audio Lossless (FLAC y WAV)**: Eliminación definitiva de opciones de compresión con pérdida al procesar formatos acústicos puros. Al seleccionar FLAC, el conversor aplica compresión lossless óptima (Nivel 8 bit-perfect o Nivel 5 rápido) sin descartar información armónica; al seleccionar WAV, aplica codificación PCM sin compresión a 24 bits Hi-Res (calidad de estudio de grabación) o 16 bits (estándar CD). Los formatos comprimidos (MP3, AAC, OGG, M4A) conservan sus perfiles de tasa de bits dedicados de hasta 320 kbps.
- **Selectores Personalizados Material 3 Expressive (`CustomSelect`)**: Sustitución integral de los selectores HTML nativos del navegador por un componente atómico estilizado (`src/shared/ui/CustomSelect.tsx`). Incorpora elevación contextual sobre tarjetas circundantes (`z-index`), apertura animada con micro-transición fluida, chevron interactivo con rotación suave, marca visual de verificación (`check`) en la opción activa, descripciones complementarias, compatibilidad con temas dinámicos claro y oscuro, y control ágil mediante teclado o clic exterior.
- **Ecualizador DSP & Conmutador de Salida Físico Unificado en Vídeo**: El botón de ecualizador en el reproductor de vídeo ahora enlaza de forma directa con el modal completo del Ecualizador DSP (`DspEqualizerModal`), unificando la experiencia con el reproductor musical. Permite alternar la salida de audio activa (altavoces, auriculares, interfaces de sonido) en plena reproducción con propagación en caliente mediante `setSinkId` y el evento en tiempo real `prisma-audio-sink-change`. Se optimizó la jerarquía visual de capas (`z-index: 100010`) para sobreponer el modal nítidamente sobre el escenario de vídeo y se añadió la salida automática de pantalla completa al invocar los ajustes acústicos.
- **Perfeccionamiento Dinámico de Notificaciones y Quick Look**: El toast flotante de captura de fotogramas a resolución nativa (`.video-snapshot-toast`) ahora ajusta su elevación automáticamente a 28 px cuando los controles del reproductor se ocultan para eliminar espacios vacíos, transicionando suavemente a 136 px al detectar actividad del usuario. Adicionalmente, se optimizó el servicio de gancho de teclado en Quick Look para una intercepción limpia y reactiva de la barra espaciadora en el Explorador de Windows.
